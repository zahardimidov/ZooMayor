...
import { $citySetSearch, $searchResultCitySets, citySetSearchChanged, citySetSearched } from '../model'

export function CitySetsDashboard() {
  const search = useUnit($citySetSearch)
  const searchChange = useUnit(citySetSearchChanged)
  const searchResults = useUnit($searchResultCitySets)
  const onSearch = useUnit(citySetSearched)
      ...
      <Accordion type="single" collapsible>
        <AccordionItem value="carousel" className="border-b-0">
          <AccordionTrigger className="flex hover:no-underline justify-start items-center gap-[20px] pl-[70px]">
            <p className="underline text-xl">НАБОРЫ КАРТ ГОРОДОВ</p>
